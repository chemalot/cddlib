...
def initialize_loggger(module_name:str, log_file:str = None):
    ...
    if log_file is not None:
        if not log_file.upper().endswith(".INI") or not os.path.exists(log_file):
            logIni = pkg_resources.resource_filename(module_name, f"log.{log_file}.ini")
            if not os.path.exists(logIni):
                logIni = pkg_resources.resource_filename(__name__, f"log.{log_file}.ini")
                if not os.path.exists(logIni):
                    logIni = pkg_resources.resource_filename(module_name, log_file)
                    if not os.path.exists(logIni):
                        logIni = pkg_resources.resource_filename(__name__, log_file)
                    if not os.path.exists(logIni):
    ...
    logConfig.fileConfig(logIni, defaults={'log_filename': "log.log"}, disable_existing_loggers=False)
